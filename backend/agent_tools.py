...
async def query_bedrock_vision(screenshot_path: Path, elements_list: list, memory: dict, page_title: str = "", page_header: str = "") -> list:
    ...
    try:
        ...
        print(f"[Vision Agent] Failed to init Bedrock inside agent_tools: {e}")
        return []
    ...
    try:
        ...
        while not page.is_closed() and loop_count < max_loops:
            ...
            try:
                ...
                await broadcast_status(websocket, "Analyzing screen visually with Bedrock Nova...")
                actions = await query_bedrock_vision(screenshot_path, unfilled_elements, memory, page_title, page_header)
                ...
                for act in actions:
                    ...
                    # Programmatic Guardrail: Lookup element and verify if Bedrock is hallucinating/wrongly mapping fields
